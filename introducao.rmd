...
Essa é o tutorial para o curso de R para jornalistas. Ao longo do dia, vocês irão aprender a:
    ...
    6. Modificar bancos de dados com o pacote tidyr.
...
No entanto, é possível importar novas funções e até mesmo bancos de dados através de pacotes. 
...
library(data.table) #nome da biblioteca sem aspas

```

Ótimo.
Agora nós temos acesso ao comando *fread* e podemos importar o nosso banco de dados.
...
Agora que já sabemos como importar bibliotecas e como acessar objetos, vamos aprender a importar banco de dados. Vamos trabalhar com o banco de jogadores da fifa de 2018.
    ...
    1. Vá em https://goo.gl/XUWoyR e baixe o arquivo "fifa2018.xls" e o arquivo "human_develop_index.csv";
    2. Salve os arquivos em uma pasta conhecida.

### Importando arquivos em .csv

CSV significa Comma Separated Values, e é um tipod e arquivo para armazenar banco de dados. Nós podemos importar arquivos .csv com o comando fread.
...
Agora vamos mostrar como importar o arquivo "human_develop_index.csv" e atribui-lo a um objeto chamado idh:

```{r, eval=F, message=FALSE, warning=FALSE}

# library(data.table)    #você já carregou essa biblioteca
...
Agora vamos importar o arquivo. Para isso, vamos usar a função *read_excel* do pacote *readxl* por meio dos comandos disponíveis na interface do R Studio. A vantagem de importar arquivos excel dessa forma é poder verificar como cada coluna será importada e alterar, se necessário.

    Vá em *File >> Import Datasets >> from Excel*

Algumas colunas estão corretamente classificadas como character, mas outras terão de ser alteradas antes de importadas. As colunas height, caps e goals estão como character, mas deveriam estar como numeric. Clique na seta ao lado do nome de cada coluna e mude a sua classe. 
...
# Importando a biblioteca readxl
library(readxl)
...
fifa2018_1 <- fifa2018 %>%  # O Pipe %>% mostra que o comando ainda não acabou 
...
Agora, digamos que vocês estão elaborando as estatísticas dos jogadores antes da copa. Com essas informações da Fifa, seria interessante saber qual é a média de gols por jogador com a camisa da seleção. Para isso vamos criar uma nova coluna que divide o número de gols (goals) pelo número de jogos pela seleção (caps). 
...
O mutate pode ser usado tanto para criar uma nova variável completamente nova quanto para sobreescrever variáveis que já existem. Vejamos um primeiro exemplo: a altura dos jogadores está em cm, que tal se ela estivesse em metros? Basta sobrescrever:
...
Agora vamos fazer um exercício no dafatrame da fifa. Vamos transformar a coluna goals em character e depois sobrescrever o objeto fifa2018_1 e transformar goals em numeric outra vez:
...
Assim como *as.character()* e *as.numeric()*, nós temos o comando *as.Date()* - sim, com D maiúsculo - que transforma um vetor em vetor de data. Para isso, temos que mostrar ao R como ele deve ler a data (onde está o dia, o mês e o ano) . 

Entendido isso, aplicar para o mutate fica simples:
...
Agora, ficar repetindo o comando (mutate) é muito chato. Lembrem-se: **programadores são preguiçosos**. No dplyr, você pode dizer o comando uma vez e colocar tudo o que diz respeito àquele comando dentro do parenteses. Entào o nosso código ficaria assim:
...
Importante: o R tem dificuldade em ler nomes com espaços e acentuação. Fora isso, é uma boa prática sempre deixar todas as letras das variáveis como minúsculas para evitar erros na hoa de digitar o código.
...
Digamos que nos interessemos em saber qual jogador tem a maior média de gols com a camisa da seleção? Ou que não queremos analisar jogadores sem gols. Como fazer?
...
# Jogadores do Irã com mais de 18 anos (nascidos antes de 2000):

ira <- fifa2018_1 %>%
  filter(team == "IR Iran" & day_of_birth < "2000-01-01")
...
gols_time <- fifa2018_1 %>%
  ...
  summarise(gols_feitos = sum(goals))     # somando os valores das linhas
...
1. Atribua a um objeto "top_5_times" o banco "fifa2018_1", depois agrupe a soma de gols por time como acabamos de demonstrar, ordene os gols em ordem decrescente com o *arrange()* e selecione as 5 primeiras linhas com o *slice()* .
...
2. Agora selecione do objeto "altura_time" apenas o team "Brazil" com o *filter()*. Qual é a altura média do time brasileiro?
...
Vamos fazer um último exemplo? Vamos descobrir quais são os 5 cluber (club) com mais jogadores na copa do mundo? Para isso vamos agrupar os jogadores por "club" e contar quantas vezes o clube aparece com o *n()* , por fim vamos ordenar em ordem descrescente com o *arrange()* e cortar os cinco primeiros com o *slice()* :
...
Agora que trabalhamos bastante com o objeto "fifa2018_1", está na hora de salva-lo para que possamos utiliza-lo na segunda parte da aula. Vamos aprender como salvar um arquivo em formato .Rdata e como exporta-lo como um csv e como uma planilha excel.
...
# Exportando como csv
#library(data.table)   - já carregamos
fwrite(fifa2018_1, file="fifa2018_1.csv")

# Exportando como excel
# library(readxl)
write.xlsx(fifa2018_1, file = "fifa2018_1.xlsx")
...
Por fim, vamos fazer um exercício em sala para fixar os conteúdos:

    1. Calcule a partir do banco fifa2018_1 o número de gols (num_gols) por time usando o group_by() e o summarise();
    2. Junte com o banco do IDH com o comando left_join() ;
    3. Selecione as colunas "team", "num_gols" e "hdi_rank_2015" com o select() ;
    4. Ordene em ordem decrescente por número de gols com o arrange(desc()) ;
    5. Atribua a um dataframe e abra com o View()
